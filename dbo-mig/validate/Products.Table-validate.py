...
sqlServerTablename = "dbo.[Products]"

df = pd.read_sql('select top 0 * from dbo.[Products]', sqlServerEngine)
sqlServerCols = sorted(df)

df = pd.read_sql('select count(*) as COUNT from dbo.[Products]', sqlServerEngine)
sqlServerCount = df.at[0,'COUNT']

sqlServerCharIntCols = list()
df = pd.read_sql("SELECT * FROM information_schema.columns WHERE table_schema = 'dbo' AND table_name = 'Products'", sqlServerEngine)
...
metatable = metadata.tables.get("Products".replace(" ", "_").lower())
databricksTablename = databricksCatalog + "." + databricksSchema + "." + "Products".replace(" ", "_").lower()
databricksCols = list()
