...
sqlServerTablename = "dbo.[EmployeeTerritories]"

df = pd.read_sql('select top 0 * from dbo.[EmployeeTerritories]', sqlServerEngine)
sqlServerCols = sorted(df)

df = pd.read_sql('select count(*) as COUNT from dbo.[EmployeeTerritories]', sqlServerEngine)
sqlServerCount = df.at[0,'COUNT']

sqlServerCharIntCols = list()
df = pd.read_sql("SELECT * FROM information_schema.columns WHERE table_schema = 'dbo' AND table_name = 'EmployeeTerritories'", sqlServerEngine)
...
metatable = metadata.tables.get("EmployeeTerritories".replace(" ", "_").lower())
databricksTablename = databricksCatalog + "." + databricksSchema + "." + "EmployeeTerritories".replace(" ", "_").lower()
databricksCols = list()
