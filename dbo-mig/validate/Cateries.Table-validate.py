...
sqlServerTablename = "dbo.[Cateries]"

df = pd.read_sql('select top 0 * from dbo.[Cateries]', sqlServerEngine)
sqlServerCols = sorted(df)

df = pd.read_sql('select count(*) as COUNT from dbo.[Cateries]', sqlServerEngine)
sqlServerCount = df.at[0,'COUNT']

sqlServerCharIntCols = list()
df = pd.read_sql("SELECT * FROM information_schema.columns WHERE table_schema = 'dbo' AND table_name = 'Cateries'", sqlServerEngine)
...
metatable = metadata.tables.get("Cateries".replace(" ", "_").lower())
databricksTablename = databricksCatalog + "." + databricksSchema + "." + "Cateries".replace(" ", "_").lower()
databricksCols = list()
