...
sqlServerTablename = "dbo.[Current Product List]"

df = pd.read_sql('select top 0 * from dbo.[Current Product List]', sqlServerEngine)
sqlServerCols = sorted(df)

df = pd.read_sql('select count(*) as COUNT from dbo.[Current Product List]', sqlServerEngine)
sqlServerCount = df.at[0,'COUNT']

sqlServerCharIntCols = list()
df = pd.read_sql("SELECT * FROM information_schema.columns WHERE table_schema = 'dbo' AND table_name = 'Current Product List'", sqlServerEngine)
...
metatable = metadata.tables.get("Current Product List".replace(" ", "_").lower())
databricksTablename = databricksCatalog + "." + databricksSchema + "." + "Current Product List".replace(" ", "_").lower()
databricksCols = list()
