...
sqlServerTablename = "dbo.[Order Details]"

df = pd.read_sql('select top 0 * from dbo.[Order Details]', sqlServerEngine)
sqlServerCols = sorted(df)

df = pd.read_sql('select count(*) as COUNT from dbo.[Order Details]', sqlServerEngine)
sqlServerCount = df.at[0,'COUNT']

sqlServerCharIntCols = list()
df = pd.read_sql("SELECT * FROM information_schema.columns WHERE table_schema = 'dbo' AND table_name = 'Order Details'", sqlServerEngine)
...
metatable = metadata.tables.get("Order Details".replace(" ", "_").lower())
databricksTablename = databricksCatalog + "." + databricksSchema + "." + "Order Details".replace(" ", "_").lower()
databricksCols = list()
