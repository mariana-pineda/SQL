...
sqlServerTablename = "dbo.[Suppliers]"

df = pd.read_sql('select top 0 * from dbo.[Suppliers]', sqlServerEngine)
sqlServerCols = sorted(df)

df = pd.read_sql('select count(*) as COUNT from dbo.[Suppliers]', sqlServerEngine)
sqlServerCount = df.at[0,'COUNT']

sqlServerCharIntCols = list()
df = pd.read_sql("SELECT * FROM information_schema.columns WHERE table_schema = 'dbo' AND table_name = 'Suppliers'", sqlServerEngine)
...
metatable = metadata.tables.get("Suppliers".replace(" ", "_").lower())
databricksTablename = databricksCatalog + "." + databricksSchema + "." + "Suppliers".replace(" ", "_").lower()
databricksCols = list()
