...
sqlServerTablename = "dbo.[Territories]"

df = pd.read_sql('select top 0 * from dbo.[Territories]', sqlServerEngine)
sqlServerCols = sorted(df)

df = pd.read_sql('select count(*) as COUNT from dbo.[Territories]', sqlServerEngine)
sqlServerCount = df.at[0,'COUNT']

sqlServerCharIntCols = list()
df = pd.read_sql("SELECT * FROM information_schema.columns WHERE table_schema = 'dbo' AND table_name = 'Territories'", sqlServerEngine)
...
metatable = metadata.tables.get("Territories".replace(" ", "_").lower())
databricksTablename = databricksCatalog + "." + databricksSchema + "." + "Territories".replace(" ", "_").lower()
databricksCols = list()
