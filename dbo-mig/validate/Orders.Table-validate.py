...
sqlServerTablename = "dbo.[Orders]"

df = pd.read_sql('select top 0 * from dbo.[Orders]', sqlServerEngine)
sqlServerCols = sorted(df)

df = pd.read_sql('select count(*) as COUNT from dbo.[Orders]', sqlServerEngine)
sqlServerCount = df.at[0,'COUNT']

sqlServerCharIntCols = list()
df = pd.read_sql("SELECT * FROM information_schema.columns WHERE table_schema = 'dbo' AND table_name = 'Orders'", sqlServerEngine)
...
metatable = metadata.tables.get("Orders".replace(" ", "_").lower())
databricksTablename = databricksCatalog + "." + databricksSchema + "." + "Orders".replace(" ", "_").lower()
databricksCols = list()
