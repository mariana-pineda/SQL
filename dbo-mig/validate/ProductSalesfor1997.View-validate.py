...
sqlServerTablename = "dbo.[Product Sales for 1997]"

df = pd.read_sql('select top 0 * from dbo.[Product Sales for 1997]', sqlServerEngine)
sqlServerCols = sorted(df)

df = pd.read_sql('select count(*) as COUNT from dbo.[Product Sales for 1997]', sqlServerEngine)
sqlServerCount = df.at[0,'COUNT']

sqlServerCharIntCols = list()
df = pd.read_sql("SELECT * FROM information_schema.columns WHERE table_schema = 'dbo' AND table_name = 'Product Sales for 1997'", sqlServerEngine)
...
metatable = metadata.tables.get("Product Sales for 1997".replace(" ", "_").lower())
databricksTablename = databricksCatalog + "." + databricksSchema + "." + "Product Sales for 1997".replace(" ", "_").lower()
databricksCols = list()
