...
sqlServerTablename = "dbo.[Order Details Extended]"

df = pd.read_sql('select top 0 * from dbo.[Order Details Extended]', sqlServerEngine)
sqlServerCols = sorted(df)

df = pd.read_sql('select count(*) as COUNT from dbo.[Order Details Extended]', sqlServerEngine)
sqlServerCount = df.at[0,'COUNT']

sqlServerCharIntCols = list()
df = pd.read_sql("SELECT * FROM information_schema.columns WHERE table_schema = 'dbo' AND table_name = 'Order Details Extended'", sqlServerEngine)
...
metatable = metadata.tables.get("Order Details Extended".replace(" ", "_").lower())
databricksTablename = databricksCatalog + "." + databricksSchema + "." + "Order Details Extended".replace(" ", "_").lower()
databricksCols = list()
