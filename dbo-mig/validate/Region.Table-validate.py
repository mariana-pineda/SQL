...
sqlServerTablename = "dbo.[Region]"

df = pd.read_sql('select top 0 * from dbo.[Region]', sqlServerEngine)
sqlServerCols = sorted(df)

df = pd.read_sql('select count(*) as COUNT from dbo.[Region]', sqlServerEngine)
sqlServerCount = df.at[0,'COUNT']

sqlServerCharIntCols = list()
df = pd.read_sql("SELECT * FROM information_schema.columns WHERE table_schema = 'dbo' AND table_name = 'Region'", sqlServerEngine)
...
metatable = metadata.tables.get("Region".replace(" ", "_").lower())
databricksTablename = databricksCatalog + "." + databricksSchema + "." + "Region".replace(" ", "_").lower()
databricksCols = list()
