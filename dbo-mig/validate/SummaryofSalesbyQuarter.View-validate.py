...
sqlServerTablename = "dbo.[Summary of Sales by Quarter]"

df = pd.read_sql('select top 0 * from dbo.[Summary of Sales by Quarter]', sqlServerEngine)
sqlServerCols = sorted(df)

df = pd.read_sql('select count(*) as COUNT from dbo.[Summary of Sales by Quarter]', sqlServerEngine)
sqlServerCount = df.at[0,'COUNT']

sqlServerCharIntCols = list()
df = pd.read_sql("SELECT * FROM information_schema.columns WHERE table_schema = 'dbo' AND table_name = 'Summary of Sales by Quarter'", sqlServerEngine)
...
metatable = metadata.tables.get("Summary of Sales by Quarter".replace(" ", "_").lower())
databricksTablename = databricksCatalog + "." + databricksSchema + "." + "Summary of Sales by Quarter".replace(" ", "_").lower()
databricksCols = list()
