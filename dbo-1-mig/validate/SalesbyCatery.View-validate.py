...
sqlServerTablename = "dbo.[Sales by Catery]"

df = pd.read_sql('select top 0 * from dbo.[Sales by Catery]', sqlServerEngine)
sqlServerCols = sorted(df)

df = pd.read_sql('select count(*) as COUNT from dbo.[Sales by Catery]', sqlServerEngine)
sqlServerCount = df.at[0,'COUNT']

sqlServerCharIntCols = list()
df = pd.read_sql("SELECT * FROM information_schema.columns WHERE table_schema = 'dbo' AND table_name = 'Sales by Catery'", sqlServerEngine)
...
metatable = metadata.tables.get("Sales by Catery".replace(" ", "_").lower())
databricksTablename = databricksCatalog + "." + databricksSchema + "." + "Sales by Catery".replace(" ", "_").lower()
databricksCols = list()
