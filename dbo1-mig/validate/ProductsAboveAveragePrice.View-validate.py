...
sqlServerTablename = "dbo.[Products Above Average Price]"

df = pd.read_sql('select top 0 * from dbo.[Products Above Average Price]', sqlServerEngine)
sqlServerCols = sorted(df)

df = pd.read_sql('select count(*) as COUNT from dbo.[Products Above Average Price]', sqlServerEngine)
sqlServerCount = df.at[0,'COUNT']

sqlServerCharIntCols = list()
df = pd.read_sql("SELECT * FROM information_schema.columns WHERE table_schema = 'dbo' AND table_name = 'Products Above Average Price'", sqlServerEngine)
...
metatable = metadata.tables.get("Products Above Average Price".replace(" ", "_").lower())
databricksTablename = databricksCatalog + "." + databricksSchema + "." + "Products Above Average Price".replace(" ", "_").lower()
databricksCols = list()
