...
sqlServerTablename = "dbo.[Employees]"

df = pd.read_sql('select top 0 * from dbo.[Employees]', sqlServerEngine)
sqlServerCols = sorted(df)

df = pd.read_sql('select count(*) as COUNT from dbo.[Employees]', sqlServerEngine)
sqlServerCount = df.at[0,'COUNT']

sqlServerCharIntCols = list()
df = pd.read_sql("SELECT * FROM information_schema.columns WHERE table_schema = 'dbo' AND table_name = 'Employees'", sqlServerEngine)
...
metatable = metadata.tables.get("Employees".replace(" ", "_").lower())
databricksTablename = databricksCatalog + "." + databricksSchema + "." + "Employees".replace(" ", "_").lower()
databricksCols = list()
