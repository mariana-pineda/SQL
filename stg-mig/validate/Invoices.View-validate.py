...
sqlServerTablename = "dbo.[Invoices]"

df = pd.read_sql('select top 0 * from dbo.[Invoices]', sqlServerEngine)
sqlServerCols = sorted(df)

df = pd.read_sql('select count(*) as COUNT from dbo.[Invoices]', sqlServerEngine)
sqlServerCount = df.at[0,'COUNT']

sqlServerCharIntCols = list()
df = pd.read_sql("SELECT * FROM information_schema.columns WHERE table_schema = 'dbo' AND table_name = 'Invoices'", sqlServerEngine)
...
metatable = metadata.tables.get("Invoices".replace(" ", "_").lower())
databricksTablename = databricksCatalog + "." + databricksSchema + "." + "Invoices".replace(" ", "_").lower()
databricksCols = list()
