...
sqlServerTablename = "dbo.[Orders Qry]"

df = pd.read_sql('select top 0 * from dbo.[Orders Qry]', sqlServerEngine)
sqlServerCols = sorted(df)

df = pd.read_sql('select count(*) as COUNT from dbo.[Orders Qry]', sqlServerEngine)
sqlServerCount = df.at[0,'COUNT']

sqlServerCharIntCols = list()
df = pd.read_sql("SELECT * FROM information_schema.columns WHERE table_schema = 'dbo' AND table_name = 'Orders Qry'", sqlServerEngine)
...
metatable = metadata.tables.get("Orders Qry".replace(" ", "_").lower())
databricksTablename = databricksCatalog + "." + databricksSchema + "." + "Orders Qry".replace(" ", "_").lower()
databricksCols = list()
