...
sqlServerTablename = "dbo.[Customers]"

df = pd.read_sql('select top 0 * from dbo.[Customers]', sqlServerEngine)
sqlServerCols = sorted(df)

df = pd.read_sql('select count(*) as COUNT from dbo.[Customers]', sqlServerEngine)
sqlServerCount = df.at[0,'COUNT']

sqlServerCharIntCols = list()
df = pd.read_sql("SELECT * FROM information_schema.columns WHERE table_schema = 'dbo' AND table_name = 'Customers'", sqlServerEngine)
...
metatable = metadata.tables.get("Customers".replace(" ", "_").lower())
databricksTablename = databricksCatalog + "." + databricksSchema + "." + "Customers".replace(" ", "_").lower()
databricksCols = list()
