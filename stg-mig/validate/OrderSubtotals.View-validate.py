...
sqlServerTablename = "dbo.[Order Subtotals]"

df = pd.read_sql('select top 0 * from dbo.[Order Subtotals]', sqlServerEngine)
sqlServerCols = sorted(df)

df = pd.read_sql('select count(*) as COUNT from dbo.[Order Subtotals]', sqlServerEngine)
sqlServerCount = df.at[0,'COUNT']

sqlServerCharIntCols = list()
df = pd.read_sql("SELECT * FROM information_schema.columns WHERE table_schema = 'dbo' AND table_name = 'Order Subtotals'", sqlServerEngine)
...
metatable = metadata.tables.get("Order Subtotals".replace(" ", "_").lower())
databricksTablename = databricksCatalog + "." + databricksSchema + "." + "Order Subtotals".replace(" ", "_").lower()
databricksCols = list()
