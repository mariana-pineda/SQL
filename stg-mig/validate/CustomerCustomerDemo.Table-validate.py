...
sqlServerTablename = "dbo.[CustomerCustomerDemo]"

df = pd.read_sql('select top 0 * from dbo.[CustomerCustomerDemo]', sqlServerEngine)
sqlServerCols = sorted(df)

df = pd.read_sql('select count(*) as COUNT from dbo.[CustomerCustomerDemo]', sqlServerEngine)
sqlServerCount = df.at[0,'COUNT']

sqlServerCharIntCols = list()
df = pd.read_sql("SELECT * FROM information_schema.columns WHERE table_schema = 'dbo' AND table_name = 'CustomerCustomerDemo'", sqlServerEngine)
...
metatable = metadata.tables.get("CustomerCustomerDemo".replace(" ", "_").lower())
databricksTablename = databricksCatalog + "." + databricksSchema + "." + "CustomerCustomerDemo".replace(" ", "_").lower()
databricksCols = list()
