...
sqlServerTablename = "dbo.[Customer and Suppliers by City]"

df = pd.read_sql('select top 0 * from dbo.[Customer and Suppliers by City]', sqlServerEngine)
sqlServerCols = sorted(df)

df = pd.read_sql('select count(*) as COUNT from dbo.[Customer and Suppliers by City]', sqlServerEngine)
sqlServerCount = df.at[0,'COUNT']

sqlServerCharIntCols = list()
df = pd.read_sql("SELECT * FROM information_schema.columns WHERE table_schema = 'dbo' AND table_name = 'Customer and Suppliers by City'", sqlServerEngine)
...
metatable = metadata.tables.get("Customer and Suppliers by City".replace(" ", "_").lower())
databricksTablename = databricksCatalog + "." + databricksSchema + "." + "Customer and Suppliers by City".replace(" ", "_").lower()
databricksCols = list()
