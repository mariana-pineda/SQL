...
sqlServerTablename = "dbo.[CustomerDemographics]"

df = pd.read_sql('select top 0 * from dbo.[CustomerDemographics]', sqlServerEngine)
sqlServerCols = sorted(df)

df = pd.read_sql('select count(*) as COUNT from dbo.[CustomerDemographics]', sqlServerEngine)
sqlServerCount = df.at[0,'COUNT']

sqlServerCharIntCols = list()
df = pd.read_sql("SELECT * FROM information_schema.columns WHERE table_schema = 'dbo' AND table_name = 'CustomerDemographics'", sqlServerEngine)
...
metatable = metadata.tables.get("CustomerDemographics".replace(" ", "_").lower())
databricksTablename = databricksCatalog + "." + databricksSchema + "." + "CustomerDemographics".replace(" ", "_").lower()
databricksCols = list()
