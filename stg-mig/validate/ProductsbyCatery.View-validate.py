...
sqlServerTablename = "dbo.[Products by Catery]"

df = pd.read_sql('select top 0 * from dbo.[Products by Catery]', sqlServerEngine)
sqlServerCols = sorted(df)

df = pd.read_sql('select count(*) as COUNT from dbo.[Products by Catery]', sqlServerEngine)
sqlServerCount = df.at[0,'COUNT']

sqlServerCharIntCols = list()
df = pd.read_sql("SELECT * FROM information_schema.columns WHERE table_schema = 'dbo' AND table_name = 'Products by Catery'", sqlServerEngine)
...
metatable = metadata.tables.get("Products by Catery".replace(" ", "_").lower())
databricksTablename = databricksCatalog + "." + databricksSchema + "." + "Products by Catery".replace(" ", "_").lower()
databricksCols = list()
