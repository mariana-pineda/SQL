...
sqlServerTablename = "dbo.[Shippers]"

df = pd.read_sql('select top 0 * from dbo.[Shippers]', sqlServerEngine)
sqlServerCols = sorted(df)

df = pd.read_sql('select count(*) as COUNT from dbo.[Shippers]', sqlServerEngine)
sqlServerCount = df.at[0,'COUNT']

sqlServerCharIntCols = list()
df = pd.read_sql("SELECT * FROM information_schema.columns WHERE table_schema = 'dbo' AND table_name = 'Shippers'", sqlServerEngine)
...
metatable = metadata.tables.get("Shippers".replace(" ", "_").lower())
databricksTablename = databricksCatalog + "." + databricksSchema + "." + "Shippers".replace(" ", "_").lower()
databricksCols = list()
