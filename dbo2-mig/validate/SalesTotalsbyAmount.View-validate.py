...
sqlServerTablename = "dbo.[Sales Totals by Amount]"

df = pd.read_sql('select top 0 * from dbo.[Sales Totals by Amount]', sqlServerEngine)
sqlServerCols = sorted(df)

df = pd.read_sql('select count(*) as COUNT from dbo.[Sales Totals by Amount]', sqlServerEngine)
sqlServerCount = df.at[0,'COUNT']

sqlServerCharIntCols = list()
df = pd.read_sql("SELECT * FROM information_schema.columns WHERE table_schema = 'dbo' AND table_name = 'Sales Totals by Amount'", sqlServerEngine)
...
metatable = metadata.tables.get("Sales Totals by Amount".replace(" ", "_").lower())
databricksTablename = databricksCatalog + "." + databricksSchema + "." + "Sales Totals by Amount".replace(" ", "_").lower()
databricksCols = list()
