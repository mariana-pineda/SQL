...
sqlServerTablename = "dbo.[Alphabetical list of products]"

df = pd.read_sql('select top 0 * from dbo.[Alphabetical list of products]', sqlServerEngine)
sqlServerCols = sorted(df)

df = pd.read_sql('select count(*) as COUNT from dbo.[Alphabetical list of products]', sqlServerEngine)
sqlServerCount = df.at[0,'COUNT']

sqlServerCharIntCols = list()
df = pd.read_sql("SELECT * FROM information_schema.columns WHERE table_schema = 'dbo' AND table_name = 'Alphabetical list of products'", sqlServerEngine)
...
metatable = metadata.tables.get("Alphabetical list of products".replace(" ", "_").lower())
databricksTablename = databricksCatalog + "." + databricksSchema + "." + "Alphabetical list of products".replace(" ", "_").lower()
databricksCols = list()
